...
dotnet restore "$DEPLOYMENT_SOURCE/src/PersonalPortfolio.sln"
exitWithMessageOnError "dotnet restore failed"

# 2. Build and publish
dotnet publish "$DEPLOYMENT_SOURCE/src/PersonalPortfolio.csproj" --output "$DEPLOYMENT_TEMP" --configuration Release
